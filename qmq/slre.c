...
exact (struct slre *r, const char **re)
{
    ...
    emit_r (r, EXACT);
    emit_r (r, old_data_size);
    emit_r (r, r->data_size - old_data_size);
}
...
quantifier (struct slre *r, int prev, int op)
{
    if (r->code [prev] == EXACT && r->code [prev + 2] > 1) {
        r->code [prev + 2]--;
        emit_r (r, EXACT);
        emit_r (r, r->code [prev + 1] + r->code [prev + 2]);
        emit_r (r, 1);
        prev = r->code_size - 3;
    }
    relocate (r, prev, 2);
...
slre_compile (struct slre *r, const char *re)
{
    ...
    emit_r (r, OPEN);  /* This will capture what matches full RE */
    emit_r (r, 0);
    while (*re != '\0')
        compile (r, &re);
    if (r->code [2] == BRANCH)
